import numpy as np

N4_EB_color_polygons = [
    np.array(
        [
            [0.18641974, -0.01650156, 0.22],
            [0.20350507, -0.00216644, 0.22],
            [0.22, -0.05, 0.22],
            [0.19, -0.05, 0.22],
        ]
    ),
    np.array(
        [
            [0.02013593, -0.00307794, 0.22],
            [0.03984131, -0.01592688, 0.22],
            [0.03, -0.03, 0.22],
            [0.008, -0.02, 0.22],
        ]
    ),
    np.array(
        [
            [0.04063031, 0.16718402, 0.22],
            [0.01565027, 0.14925183, 0.22],
            [-0.00344147, 0.16541647, 0.22],
            [0.025, 0.20, 0.22],
        ]
    ),
    np.array(
        [
            [0.20267981, 0.14451691, 0.22],
            [0.18416046, 0.16943716, 0.22],
            [0.195, 0.210, 0.22],
            [0.250, 0.130, 0.22],
        ]
    ),
]

N4_EB_delete_polygons = [
    np.array(
        [
            [0.0, 0.0, -0.20],
            [-0.1, -0.10, -0.20],
            [-0.05, -0.05, -0.20],
        ]
    )
]


N6_color_polygons = np.array(
    [
        [
            [-0.005, 0.050, 0.220],
            [-0.005, 0.100, 0.220],
            [0.015, 0.100, 0.220],
            [0.015, 0.050, 0.220],
        ],
        [
            [0.21, 0.050, 0.220],
            [0.21, 0.100, 0.220],
            [0.29, 0.050, 0.220],
            [0.29, 0.100, 0.220],
        ],
        [
            [0.15, -0.018, 0.220],
            [0.175, -0.0036, 0.220],
            [0.185, -0.028, 0.220],
            [0.16, -0.036, 0.220],
        ],
        [
            [0.053, -0.0053, 0.220],
            [0.074, -0.018, 0.220],
            [0.074, -0.026, 0.220],
            [0.021, -0.051, 0.220],
        ],
        [
            [0.053, 0.15, 0.220],
            [0.074, 0.16, 0.220],
            [0.082, 0.19, 0.220],
            [0.031, 0.19, 0.220],
        ],
        [
            [0.15, 0.162, 0.225],
            [0.17, 0.152, 0.225],
            [0.19, 0.19, 0.225],
            [0.14, 0.20, 0.225],
        ],
    ]
)

N6_delete_polygons = [
    np.array(
        [
            [0.0, 0.0, -0.20],
            [-0.1, -0.10, -0.20],
            [-0.05, -0.05, -0.20],
        ]
    )
]

N5_color_polygons = np.array(
    [
        [
            [0.13, -0.015, 0.220],  # corners
            [0.15, -0.0087, 0.220],
            [0.16, -0.051, 0.220],
            [0.13, -0.056, 0.220],
        ],
        [
            [0.035, 0.031, 0.220],
            [0.047, 0.014, 0.220],
            [-0.035, -0.03, 0.220],
            [-0.027, 0.015, 0.220],
        ],
        [
            [0.048, 0.13, 0.220],
            [0.035, 0.12, 0.220],
            [-0.020, 0.05, 0.220],
            [0.017, 0.17, 0.220],
        ],
        [
            [0.155, 0.155, 0.220],
            [0.13, 0.16, 0.220],
            [0.14, 0.19, 0.220],
            [0.17, 0.17, 0.220],
        ],
        [
            [0.2, 0.062, 0.220],
            [0.2, 0.087, 0.220],
            [0.22, 0.084, 0.220],
            [0.22, 0.065, 0.220],
        ],
        [
            [0.043, 0.02, 0.210],  # sides
            [0.14, -0.012, 0.210],
            [0.13, -0.023, 0.210],
            [0.042, 0.011, 0.210],
        ],
        [
            [0.14, -0.0084, 0.210],
            [0.2, 0.07, 0.210],
            [0.21, 0.064, 0.210],
            [0.15, -0.016, 0.210],
        ],
        [
            [0.039, 0.12, 0.210],
            [0.039, 0.026, 0.210],
            [0.027, 0.023, 0.210],
            [0.027, 0.13, 0.210],
        ],
        [
            [0.14, 0.16, 0.210],
            [0.045, 0.13, 0.210],
            [0.038, 0.14, 0.210],
            [0.14, 0.17, 0.210],
        ],
        [
            [0.2, 0.078, 0.210],
            [0.14, 0.16, 0.210],
            [0.15, 0.17, 0.210],
            [0.21, 0.083, 0.210],
        ],
    ]
)

N5_delete_polygons = [
    np.array(
        [
            [0.1529166, -0.01611024, 0.20],
            [0.19471136, -0.02285242, 0.20],
            [0.21088745, -0.01519365, 0.20],
            [0.22872858, 0.04915744, 0.20],
            [0.21668607, 0.06462979, 0.20],
            [0.20088504, 0.06146107, 0.20],
            [0.15061945, -0.00818526, 0.20],
        ]
    ),
    np.array(
        [
            [0.04558737, 0.01368767, 0.20],
            [0.03764117, -0.0148172, 0.20],
            [0.05197833, -0.02188771, 0.20],
            [0.06824371, -0.02205147, 0.20],
            [0.13191617, -0.02314828, 0.20],
            [0.128108, -0.0132541, 0.20],
        ]
    ),
    np.array(
        [
            [0.03529871, 0.11781342, 0.2],
            [0.02415088, 0.12590814, 0.2],
            [0.02184369, 0.04133041, 0.2],
            [0.02177271, 0.02243735, 0.2],
            [0.03464105, 0.03030618, 0.2],
        ]
    ),
    np.array(
        [
            [0.04610632, 0.1369007, 0.2],
            [0.04788623, 0.13487723, 0.2],
            [0.12851248, 0.15966498, 0.2],
            [0.13201035, 0.16809216, 0.2],
            [0.06820551, 0.17120758, 0.2],
            [0.05513959, 0.17119415, 0.2],
            [0.03204877, 0.16839441, 0.2],
            [0.02287036, 0.15415363, 0.2],
        ]
    ),
    np.array(
        [
            [0.15061722, 0.15597513, 0.2],
            [0.20150677, 0.0846152, 0.2],
            [0.20988593, 0.08422189, 0.2],
            [0.21821747, 0.08422278, 0.2],
            [0.20388962, 0.15890372, 0.2],
            [0.20131342, 0.16644717, 0.2],
            [0.16270276, 0.16677191, 0.2],
        ]
    ),
]
